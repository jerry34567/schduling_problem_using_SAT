...
class SatSolver
{
   public : 
      ...
      void addCondition1(Var v1, Var v2){
         // cout << "clause: " << v1 << "+" << v2 << endl;
         vec<Lit> lits;
         Lit l1 = ~Lit(v1);
         Lit l2 = Lit(v2);
         lits.push(l1);
      ...
      void addCondition2(Var v1, Var v2, Var v3, Var v4, Var v5){
         // cout << 2 << " ";
         vec<Lit> lits;
         Lit l1 = ~Lit(v1);
         Lit l2 = ~Lit(v2);
      ...
      void addCondition3(Var v1, Var v2, Var v3, Var v4, Var v5){
         // cout << 3 << " ";
         vec<Lit> lits;
         Lit l1 = Lit(v1);
         Lit l2 = Lit(v2);
      ...
      void addCondition4(Var vf, Var v1, Var v2, Var v3){
         // cout << 4 << " ";
         vec<Lit> lits;
         Lit lf = Lit(vf);
         Lit l1 = Lit(v1);
         Lit l2 = Lit(v2);
         Lit l3 = Lit(v3);
         lits.push(~lf); lits.push( l1); lits.push( l2); lits.push( l3);
         _solver->addClause(lits); lits.clear();
         lits.push( lf); lits.push(~l1);
         _solver->addClause(lits); lits.clear();
         lits.push( lf); lits.push(~l2);
         _solver->addClause(lits); lits.clear();
         lits.push( lf); lits.push(~l3);
         _solver->addClause(lits); lits.clear();
      }

      void addCondition5(vector<Var>& vec_var, int y, int sum, int index){
         Var V = newVar();
         map[make_pair(0,0)] = V;
         _solver->addUnit(Lit(V));
         tempCondition5(vec_var,y,sum,index);
         // vector<Var> temp_var;
         // cout << "temp size: " << vec_var.size() << endl;
         // tempCondition5(temp_var, vec_var, 0, vec_var.size()-1, y+1);
         
      }

      void tempCondition5(vector<Var>& vec_var, int y, int sum, int index){
         ...
         if(index == vec_var.size()-1){
         ...
         else if(sum == vec_var.size() - y - 1){
            if(!map.count({sum,index+1})){
               Var V = newVar();
               map[make_pair(sum,index+1)] = V;               
               tempCondition5(vec_var,y,sum,index+1);
            }
            vec<Lit> lits;
            Lit lx = Lit(map[{sum,index}]);
            Lit ls = ~Lit(vec_var[index]);
            ...
            if(!map.count({sum+1,index+1})){
               ...
               tempCondition5(vec_var,y,sum+1,index+1);
            }
            vec<Lit> lits;
            Lit lx = Lit(map[{sum,index}]);
            Lit ls = ~Lit(vec_var[index]);
      ...
      void printClause(){
            cout << "clause number: " << _solver->nClauses() << endl;
      }
